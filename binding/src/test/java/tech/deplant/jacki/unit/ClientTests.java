...
import tech.deplant.jacki.binding.Client;
...
public class ClientTests {
	...
	@Test
	public void sdk_version_equals_constant() throws TvmSdkException, ExecutionException, InterruptedException {
		int ctxId = TestEnv.newContextEmpty();
		assertEquals(DefaultLoader.EVER_SDK_VERSION, Client.version(ctxId).get().version());
	}

	@Test
	public void get_version_snippet() throws TvmSdkException {
		int ctxId = TestEnv.newContextEmpty();
		var asyncResult = Client.version(ctxId);
		var syncResult = TvmSdk.await(asyncResult);
		System.out.println("EVER-SDK Version: " + syncResult.version());
	}

	@Test
	public void api_reference_version_equals_constant() throws TvmSdkException, ExecutionException, InterruptedException {
		int ctxId = TestEnv.newContextEmpty();
