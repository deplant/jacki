 ...
 */
public class TvmSdkContext implements tc_response_handler_t.Function {

	private final static System.Logger logger = System.getLogger(TvmSdkContext.class.getName());
	 ...
	 */
	public TvmSdkContext(int id, Client.ClientConfig clientConfig) {
		this.id = id;
	...
	public <R, P, AP, AR> CompletableFuture<R> callAsync(final String functionName,
		...
		try {
			...
			logger.log(System.Logger.Level.TRACE,
			           () -> TvmSdk.LOG_FORMAT.formatted(this.id, requestId, functionName, "SEND", paramsJson));
		}
		if (!hasResponse) {
			...
			try {
				...
				logger.log(System.Logger.Level.ERROR,
				           () -> "CTX:%d REQ:%d EVER-SDK Response deserialization failed! %s".formatted(this.id,
				                                                                                        ...
				                                                                                        ex2.toString()));
				request.responseFuture()
				       .completeExceptionally(new TvmSdkException(new TvmSdkException.ErrorResult(-500,
	...
	private void addError(int requestId, final RequestData request, final String responseString) {
		if (request.responseFuture() instanceof CompletableFuture<?> future) {
			try {
				...
				TvmSdkException.ErrorResult sdkResponse = JsonContext.SDK_JSON_MAPPER()
				                                                      .readValue(responseString,
				                                                                 TvmSdkException.ErrorResult.class);
				future.completeExceptionally(new TvmSdkException(sdkResponse));
				...
				logger.log(System.Logger.Level.ERROR,
				           () -> "CTX:%d REQ:%d EVER-SDK Error deserialization failed! %s".formatted(this.id,
				                                                                                     requestId,
				                                                                                     ex1.toString()));
				future.completeExceptionally(new TvmSdkException(new TvmSdkException.ErrorResult(-500,
