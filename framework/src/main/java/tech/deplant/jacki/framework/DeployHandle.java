...
import tech.deplant.jacki.framework.template.AbstractTemplate;
import tech.deplant.jacki.framework.template.Template;
...
public record DeployHandle<RETURN extends AbstractContract>(Class<RETURN> clazz,
	...
	public String encodeInitialDataBase64() throws TvmSdkException {
		var result = TvmSdk.await(Abi.encodeInitialData(sdk(),
		                                                 template().abi().ABI(),
	 ...
	 */
	public Abi.DeploySet toDeploySet() throws TvmSdkException {
	 ...
	 * @throws TvmSdkException the ever sdk exception
	 */
	public Abi.CallSet toConstructorCallSet() throws TvmSdkException {
	 ...
	 * @throws TvmSdkException the ever sdk exception
	 */
	public Address toAddress() throws TvmSdkException {
		return new Address(TvmSdk.await(Abi.encodeMessage(sdk(),
		                                                   template().abi().ABI(),
		                                                   null,
		                                                   toDeploySet(),
		                                                   null,
	 ...
	 * @throws TvmSdkException the ever sdk exception
	 */
	public RETURN deployWithGiver(GiverContract giver, BigInteger value) throws TvmSdkException {
		...
		try {
			...
			throw new TvmSdkException(new TvmSdkException.ErrorResult(-400, "EVER-SDK call interrupted!"), e);
		} catch (TimeoutException e) {
			logger.log(System.Logger.Level.ERROR, () -> "Wait for giver funds timeout! Message: " + e.getMessage());
			throw new TvmSdkException(new TvmSdkException.ErrorResult(-402, "EVER-SDK Execution expired on Timeout!"),
			                           e);
		} catch (ExecutionException e) {
			logger.log(System.Logger.Level.ERROR, () -> "Wait for giver funds failed! Message: " + e.getMessage());
			throw new TvmSdkException(new TvmSdkException.ErrorResult(-400, "EVER-SDK call failed!"), e);
		}
	 ...
	 * @throws TvmSdkException the ever sdk exception
	 */
	public RETURN deploy() throws TvmSdkException {
		var address = toAddress();
		return deploy(address);
	}

	private RETURN deploy(Address address) throws TvmSdkException {
		TvmSdk.await(Processing.processMessage(sdk(),
		                                        template().abi().ABI(),
		                                        address.makeAddrStd(),
		                                        toDeploySet(),
