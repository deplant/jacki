...
public record Account(String id,
	 ...
	 */
	public static Account ofAddress(int sdk, String addressString) throws TvmSdkException {
		Map<String, Object> filter = new HashMap<>();
		filter.put("id", new GraphQLFilter.In(new String[]{addressString}));
		Net.ResultOfQueryCollection result = TvmSdk.await(Net.queryCollection(sdk,
		                                                         "accounts",
	 ...
	 * @throws TvmSdkException the ever sdk exception
	 */
	public static Account ofAddress(int sdk, Address address) throws TvmSdkException {
	...
	public static List<Account> ofAddressList(int sdk,
	                                          String... addresses) throws TvmSdkException {
		Map<String, Object> filter = new HashMap<>();
		filter.put("id",
		           new GraphQLFilter.In(addresses));
		return Arrays
				.stream(TvmSdk.await(Net.queryCollection(sdk,
				                            "accounts",
				                            JsonContext.ABI_JSON_MAPPER().valueToTree(filter),
				                            "id acc_type balance boc data data_hash code code_hash init_code_hash last_trans_lt",
				                            null,
